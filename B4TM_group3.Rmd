...
Model_accuracies <- data.frame(kNN.train = knn_model_evaluation$knn_evaluation$Accuracy_train,
                               ...
                               Linear_regression.train = lr_model_evaluation$lr_evaluation$Accuracy_train,
                               Linear_regression.test = lr_model_evaluation$lr_evaluation$Accuracy_test)
...
ggplot(data = Model_accuracies) +
  ...
  geom_jitter(data = Model_accuracies[c(seq(101,200), seq(301, 400), seq(501,600)),], mapping = aes(x = Assessment_method, y = Accuracy), width = 0.2, color = "Blue") +
  geom_jitter(data = Model_accuracies[c(seq(1,100), seq(201, 300), seq(401,500)),], mapping = aes(x = Assessment_method, y = Accuracy), width = 0.2, color = "Green") +
  ylim(0:1) +
  geom_line(mapping = aes(x = Assessment_method, y = baseline_accuracy, group = 1), color = "Red") +
  ggtitle("Accuracy For Different Models") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 45, margin = margin(t= 30), colour = "Black"))
```


```{r}
par(mfrow = c(1,2))
ggplot(data = svm_coef) +
  geom_bar(mapping = aes(x = as.factor(DNA_regions), y = Coefficients), stat = "identity", fill = "Blue") +
  xlab("DNA region") +
  ylab("Coefficient") +
  theme_bw() +
  ggtitle("SVM")
