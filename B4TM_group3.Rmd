...
Model_accuracies$sum <- as.factor(Model_accuracies$Assessment_method)
Model_accuracies <- Model_accuracies %>%
  separate(Assessment_method, c('model', 'dataset'), sep='[.]') %>%
  relocate(sum)

test_accuracies <- Model_accuracies %>%
  filter(dataset=='test')

training_accuracies <- Model_accuracies %>%
  filter(dataset=='train')

Model_accuracies$dataset <- factor(Model_accuracies$dataset, ordered=T, levels=c('train', 'test'))


  ggplot(data=Model_accuracies, mapping=aes(model, Accuracy)) +
