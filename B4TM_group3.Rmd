...
# Inner loop cross validation and model training
...
test_index_start <- seq(1, dim(train_data_array)[1], dim(train_data_array)[1]/cv_outer)
test_index_end <- seq(dim(train_data_array)[1]/cv_outer, dim(train_data_array)[1], dim(train_data_array)[1]/cv_outer)
...
test_index <- seq(test_index_start[i], test_index_end[i], 1)
...
fit.knn <- kNN_models[[best_knn_models$n_features[1]]]

kNN_models_outer[[i]] <- fit.knn
